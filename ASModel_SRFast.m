...
plot(time,L*cos(thetasim_interp(:,1)),'-',time,L*sin(thetasim_interp(:,1)),'--')
figure;
plot(t,L*cos(y(:,1)),'-',t,L*sin(y(:,1)),'--'); % L cos =y; Lsin=x
figure;
plot(time,L*cos(thetasim_interp(:,1)),'-',time,L*sin(thetasim_interp(:,1)),'--',time,L*cos(y(:,1)),'-',time,L*sin(y(:,1)),'--')
data.yexp=yexp;
data.time=time;
